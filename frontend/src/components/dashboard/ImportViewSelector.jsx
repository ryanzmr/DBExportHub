...
const ImportViewSelector = ({ 
  ...
  useEffect(() => {
    const fetchViews = async () => {
      ...
      try {
        ...
        setViews(response.import || []);
        setError(null);
        
        // If no view is selected and views are available, select the first one
        if (!value && response.import && response.import.length > 0) {
          onChange(response.import[0].id);
        }
      } catch (err) {
  ...
  useEffect(() => {
    const validateView = async () => {
      ...
      setValidationStatus('pending');
        ...
        const result = await validateViewExists(connectionDetails, value, 'import');
    ...
    validateView();
  }, [value, connectionDetails]);  // Render validation chip based on status
  const renderStatusChip = () => {
    ...
    return (
      <Box sx={{ display: 'flex', alignItems: 'center', height: 40, pl: 1, maxWidth: '380px' }}>
        <CircularProgress size={12} sx={{ mr: 1 }} />
        <Typography variant="body2" fontSize="0.75rem">Loading views...</Typography>
      </Box>
    );
  }
  ...
  const selectedView = views.find(v => v.id === value);  return (
    <Box sx={{ width: '100%', maxWidth: '380px' }}>      
      <Box sx={{ mb: 0.75, display: 'flex', alignItems: 'center', gap: 0.5 }}>
        ...
        >
          {label}
          <Tooltip title="Select a database view to import data from">
      ...
      <StyledFormControl error={validationStatus === 'invalid'}>
        <StyledSelect
          ...
          displayEmpty
          IconComponent={KeyboardArrowDown}          startAdornment={
            <ViewColumn 
              sx={{ 
                color: 'primary.main', 
                mr: 0.75, 
          ...
          endAdornment={validating && <CircularProgress size={12} sx={{ mr: 1 }} />}renderValue={() => (
      ...
      </StyledFormControl>      {validationError && (
        <Alert 
          severity="error"
...
export default ImportViewSelector;
